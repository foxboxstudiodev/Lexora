import { Level } from './types';

export const starterLevels: Level[] = [
  {
    id: 1,
    language: 'en',
    letters: ['S', 'T', 'A', 'R'],
    mainWords: [
      { word: 'STAR', row: 0, col: 0, direction: 'across' },
      { word: 'ART', row: 0, col: 2, direction: 'down' },
      { word: 'RAT', row: 2, col: 0, direction: 'across' },
    ],
    bonusWords: ['TAR', 'SAT'],
    difficulty: 'easy',
    themeId: 'dawn-garden',
    rewardCoins: 15,
  },
  {
    id: 2,
    language: 'en',
    letters: ['P', 'L', 'A', 'N', 'E'],
    mainWords: [
      { word: 'PLANE', row: 0, col: 0, direction: 'across' },
      { word: 'LANE', row: 0, col: 1, direction: 'down' },
      { word: 'PAN', row: 0, col: 0, direction: 'down' },
    ],
    bonusWords: ['PLAN', 'LEAN', 'PALE', 'LEAP'],
    difficulty: 'easy',
    themeId: 'dawn-garden',
    rewardCoins: 18,
  },
  {
    id: 3,
    language: 'en',
    letters: ['S', 'T', 'O', 'N', 'E'],
    mainWords: [
      { word: 'STONE', row: 0, col: 0, direction: 'across' },
      { word: 'TONE', row: 0, col: 1, direction: 'down' },
      { word: 'NOTE', row: 0, col: 3, direction: 'down' },
    ],
    bonusWords: ['ONES', 'TOES', 'SENT', 'NEST'],
    difficulty: 'normal',
    themeId: 'dawn-garden',
    rewardCoins: 22,
  },
  {
    id: 1,
    language: 'es',
    letters: ['S', 'O', 'L', 'A'],
    mainWords: [
      { word: 'SOLA', row: 0, col: 0, direction: 'across' },
      { word: 'SOL', row: 0, col: 0, direction: 'down' },
      { word: 'OLA', row: 0, col: 1, direction: 'down' },
    ],
    bonusWords: ['LAS', 'SAL'],
    difficulty: 'easy',
    themeId: 'sunny-coast',
    rewardCoins: 15,
  },
  {
    id: 2,
    language: 'es',
    letters: ['M', 'A', 'R', 'E', 'S'],
    mainWords: [
      { word: 'MARES', row: 0, col: 0, direction: 'across' },
      { word: 'MAR', row: 0, col: 0, direction: 'down' },
      { word: 'SER', row: 0, col: 4, direction: 'down' },
    ],
    bonusWords: ['MAS'],
    difficulty: 'easy',
    themeId: 'sunny-coast',
    rewardCoins: 18,
  },
  {
    id: 3,
    language: 'es',
    letters: ['C', 'A', 'M', 'I', 'N', 'O'],
    mainWords: [
      { word: 'CAMINO', row: 0, col: 0, direction: 'across' },
      { word: 'MANO', row: 0, col: 2, direction: 'down' },
      { word: 'CIMA', row: 0, col: 0, direction: 'down' },
    ],
    bonusWords: ['AMO', 'COMA'],
    difficulty: 'normal',
    themeId: 'sunny-coast',
    rewardCoins: 22,
  },
  {
    id: 1,
    language: 'ru',
    letters: ['Л', 'Е', 'С', 'А'],
    mainWords: [
      { word: 'ЛЕСА', row: 0, col: 0, direction: 'across' },
      { word: 'ЛЕС', row: 0, col: 0, direction: 'down' },
      { word: 'ЕЛА', row: 0, col: 1, direction: 'down' },
    ],
    bonusWords: ['СЕЛ'],
    difficulty: 'easy',
    themeId: 'mystic-forest',
    rewardCoins: 15,
  },
  {
    id: 2,
    language: 'ru',
    letters: ['М', 'О', 'Р', 'Е'],
    mainWords: [
      { word: 'МОРЕ', row: 0, col: 0, direction: 'across' },
      { word: 'МОР', row: 0, col: 0, direction: 'down' },
      { word: 'РОМ', row: 0, col: 2, direction: 'down' },
    ],
    bonusWords: ['МЕР'],
    difficulty: 'easy',
    themeId: 'crystal-lake',
    rewardCoins: 18,
  },
  {
    id: 3,
    language: 'ru',
    letters: ['К', 'А', 'Р', 'Т', 'А'],
    mainWords: [
      { word: 'КАРТА', row: 0, col: 0, direction: 'across' },
      { word: 'КАРА', row: 0, col: 0, direction: 'down' },
      { word: 'АРКА', row: 0, col: 1, direction: 'down' },
    ],
    bonusWords: ['АКТ'],
    difficulty: 'normal',
    themeId: 'mystic-forest',
    rewardCoins: 22,
  },
  {
    id: 1,
    language: 'tr',
    letters: ['K', 'A', 'L', 'E'],
    mainWords: [
      { word: 'KALE', row: 0, col: 0, direction: 'across' },
      { word: 'KAL', row: 0, col: 0, direction: 'down' },
      { word: 'EL', row: 0, col: 3, direction: 'down' },
    ],
    bonusWords: ['AL'],
    difficulty: 'easy',
    themeId: 'ancient-city',
    rewardCoins: 15,
  },
  {
    id: 2,
    language: 'tr',
    letters: ['D', 'E', 'N', 'İ', 'Z'],
    mainWords: [
      { word: 'DENİZ', row: 0, col: 0, direction: 'across' },
      { word: 'DEN', row: 0, col: 0, direction: 'down' },
      { word: 'İZ', row: 0, col: 3, direction: 'down' },
    ],
    bonusWords: ['DİN'],
    difficulty: 'easy',
    themeId: 'crystal-lake',
    rewardCoins: 18,
  },
  {
    id: 3,
    language: 'tr',
    letters: ['K', 'İ', 'T', 'A', 'P'],
    mainWords: [
      { word: 'KİTAP', row: 0, col: 0, direction: 'across' },
      { word: 'KİT', row: 0, col: 0, direction: 'down' },
      { word: 'TAP', row: 0, col: 2, direction: 'down' },
    ],
    bonusWords: ['KAT', 'KAP', 'İP'],
    difficulty: 'normal',
    themeId: 'ancient-city',
    rewardCoins: 22,
  },
];
