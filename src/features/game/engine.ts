...
export function validateGuess(level: Level, rawGuess: string, foundWords: Set<string>, foundBonusWords: Set<string>): GuessResult {
  ...
  if (word.length < 2) return { status: 'invalid', word };

  const main = level.mainWords.map((item) => normalizeWord(item.word));
  const bonus = level.bonusWords.map(normalizeWord);
...
export function isLevelComplete(level: Level, foundWords: Set<string>): boolean {
  return level.mainWords.every((item) => foundWords.has(normalizeWord(item.word)));
}
